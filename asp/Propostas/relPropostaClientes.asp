...
<%dim nomeCliente, Situacao, sql, rs, vcor, cor
nomeCliente = trim(replace(request("nomeCliente"),"'","''"))
...
sub principal
	...
	if request("isPostBack") then
		...
		sql = "SELECT cnpj, nome, site, dataCadastro FROM parceiro WHERE Ativo = " & situacao & _
				" AND tipo = 0"	
				if nomeCliente<>"" then sql = sql & " AND nome like '" & nomeCliente & "%'"
				
					
		set rs = conn.execute(sql)
		%>
			<label class="titulo">Relatório de Clientes</label><br>
			<label class="subtitulo">Situação: <%=iif(situacao=0,"Deletados","Ativos")%></label><br>
			<label class="subtitulo">Pesquisar por: <%=iif(nomeCliente="","Todos","""" & nomeCliente & """")%></label>
...
<%sub Impressao
	response.write request.ServerVariables("SCRIPT_NAME") & "?nomeCliente=" & nomeCliente & "&situacao=" & situacao & "&imp=1&isPostBack=1&time=" &time
end sub
...
<table>
	...
	<tr>
		...
		<input type="text" size="20" value="<%=nomeCliente%>" name="nomeCliente" class="caixa">
		</td>
