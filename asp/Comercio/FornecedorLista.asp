...
if acao = "edit"  then
	response.Redirect("FornecedorDetalhes.asp?tipo=1&idParceiro=" & request.Form("optLinha") & "&acao=" & acao)
elseif acao = "add" then
	response.Redirect("cadFornecedor.asp?redir=FornecedorLista.asp&tipo=1&idParceiro=" & request.Form("optLinha") & "&acao=" & acao)
...
sub FormLista
	...
	sql = "Select id, nome, DataCadastro from Parceiro where tipo = 1 and ativo = " & situacao
	...
	<label class="titulo">Fornecedores</label>
	<br>
	<label class="subtitulo">Lista dos Fornecedores</label>
	<%
